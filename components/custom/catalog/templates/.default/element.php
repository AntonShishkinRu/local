...
if ($basketUserId > 0){
   $viewedIterator = \Bitrix\Catalog\CatalogViewedProductTable::getList(array(
	  ...
	  'filter' => array('=FUSER_ID' => $basketUserId, '=SITE_ID' => SITE_ID, '!PRODUCT_ID' => $ElementID, '!ELEMENT_ID' => $ElementID),
   ...
   while ($arFields = $viewedIterator->fetch()){
    ...
    if (count($arViewed)) { 
		...
		$arrFilterViewed = array("ID" => $arViewed);
		
		$APPLICATION->IncludeComponent(
			"bitrix:catalog.section",
			"slider",
			array(
				"PAGER_TITLE" => "РАНЕЕ ПРОСМОТРЕННЫЕ ТОВАРЫ",
				"WIDTH_BORDER" => "1",
				"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
				"IBLOCK_ID" => $arParams["IBLOCK_ID"],
				"ELEMENT_SORT_FIELD" => 'id',
				"ELEMENT_SORT_ORDER" => $arViewed,
...
<section class="contacts_block block">
	<div class="cont">
		...
		<div class="socials">
			...
			Array(
				"AREA_FILE_SHOW" => "file",
				"PATH" => "/local/include/need_consult.php"
			));?>
			<?	
