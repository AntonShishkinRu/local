...
$dataClassOffers = \Bitrix\Iblock\Iblock::wakeUp(OFFERS_ID)->getEntityDataClass();
...
foreach ($this->basketItems as $row)
{
	...
	// Получаем имя позиции по родительскому товару предложения
	$offer = $dataClassOffers::getList([
		'filter' => ['ID' => $rowData['PRODUCT_ID']],
		'select' => ['CML2_LINK_' => 'CML2_LINK'],
		'limit' => 1
	])->fetch();
	if (!empty($offer['CML2_LINK_IBLOCK_GENERIC_VALUE'])) {
		$el = $dataClassCatalog::getList([
			'filter' => ['ID' => $offer['CML2_LINK_IBLOCK_GENERIC_VALUE']],
			'select' => ['NAME'],
			'limit' => 1
		])->fetch();
		if($el['NAME']) {
			$rowData['NAME'] = $el['NAME'];
		}
	}
