...
?>
	...
	<?	$dataClassCatalog = \Bitrix\Iblock\Iblock::wakeUp(CAT_ID)->getEntityDataClass();
		...
		{ 
			...
			<div class="order">
				<div class="head">
					<div class="number"><?=$order['ORDER']['ACCOUNT_NUMBER'];?></div>
				...
				<div class="data">
					<div class="cart_info">
						<div class="titles">
							<div class="info">ТОВАР</div>
							<div class="price">ЦЕНА</div>
							<div class="amount">КОЛИЧЕСТВО</div>
							<div class="price total">СУММА</div>
						</div>
						<? foreach ($order["BASKET_ITEMS"] as $item){ 
							$el = $dataClassCatalog::getList([
								'filter' => ['ID' => $item['PRODUCT_ID']],
								'select' => ['PREVIEW_PICTURE', 'ARTICLE_VAL' => 'ARTICLE.VALUE', 'BRAND_' => 'BRAND.ITEM'],
								'limit' => 1
							])->fetch();?>
							<div class="product">
								<a href="<?=$item["DETAIL_PAGE_URL"]?>" class="thumb">
									<? if ($pic = intval($el['PREVIEW_PICTURE'])) { ?><img src="<?=CFile::GetPath($pic);?>" loading="lazy"><? } ?>
								</a>

								<div class="info">
									<div class="brand">
										<?if ($el['BRAND_VALUE']) { ?><a href="/catalog/<?=$el['BRAND_XML_ID'];?>/"><?=$el['BRAND_VALUE'];?></a><?}?>
									</div>
									...
									<div class="articul"><?if ($el['ARTICLE_VAL']) { ?>Aртикул: <?=$el['ARTICLE_VAL'];?><? } ?></div>
								</div>

								<div class="price" data-column="ЦЕНА">
									<?=CurrencyFormat($item["PRICE"], "RUB");?>
									<?if ($item["BASE_PRICE"] != $item["PRICE"]){?><span class="old"><?=CurrencyFormat($item["BASE_PRICE"], "RUB");?></span><?}?>
								</div>

								<div class="amount" data-column="КОЛИЧЕСТВО">
									<span><?=$item['QUANTITY']?></span>
								</div>

								<div class="price total" data-column="СУММА">
									<?=CurrencyFormat($item["PRICE"]*$item['QUANTITY'], "RUB");?>
								</div>
							</div>
						<? } ?>
