...
<style>
	.bx-soa-section-title-container, .bx-soa-cart-total-button-container {
		display: none;
	}
	...
	#bx-soa-delivery .arr svg {
		width: 100%;
		height: 100%;
	}
	
	.form #bx-soa-delivery select ~ .arr {
		top: 20px;
		bottom: unset;
	}
	...
	.bx-soa-pp-inner-ps .bx-soa-pp-company-image {
		width: 60px;
		height: 50px;
	}
</style>
...
else
{
	...
	?>
<section class="checkout_info block" id="checkout">
	<div class="cont small">
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data" class="form">
		<?php
		...
		<div id="bx-soa-order" style="opacity: 0">
			...
			<div class="bx-soa">
				...
				<!--	BUYER PROPS BLOCK	-->
				<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active section">
			<!--		<div class="bx-soa-section-title-container">
						<h2 class="bx-soa-section-title col-sm-9">
				...
				<!--	REGION BLOCK	-->
				<div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-active section">
				<!--	<div class="bx-soa-section-title-container">
						<h2 class="bx-soa-section-title col-sm-9">
				...
				?>
					<!--	PAY SYSTEMS BLOCK	-->
					<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active section payment_methods">
					<!--	<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
					...
					<!--	DELIVERY BLOCK	-->
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active line width2of3" <?=($hideDelivery ? 'style="display:none"' : '')?>>
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
						...
						<div class="bx-soa-section-content container-fluid columns_delivery"></div>
					</div>
					<!--	PICKUP BLOCK	-->
					<div id="bx-soa-pickup" data-visited="false" class="bx-soa-section section" style="display:none">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
				...
				?>
					<!--	BASKET ITEMS BLOCK	-->
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active section">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-count"></span><?=$arParams['MESS_BASKET_BLOCK_NAME']?>
							</h2>
	...
	<script>
		...
		$(function(){
			$('.checkout_btn').on('click', function(){
				let curr_delivery_id = $('input[name="DELIVERY_ID"]:checked').val();
				let valid_flag = 1;
				let form_obj = $('#bx-soa-order-form');
				form_obj.find('label.error_custom').detach();
				$('.error_custom_ac').detach();
				
				if ($('input[name="aggreement_cart"]').val()) {
					if ($('input[name="aggreement_cart"]').is(":checked"));
					else {
						$('input[name="aggreement_cart"]').parent().append('<label class="error error_custom error_custom_ac">Согласие обязательно</label>');
						valid_flag = 0;	
					}
				}
				...
				/*
				// ПВЗ для СДЭК
				if (curr_delivery_id == '24') {
					if ($('#IPOLSDEK_injectHere a').text() == 'Выбрать пункт самовывоза') {
						view_msg("Пожалуйста, выберите конкретный пункт самовывоза.", "", 300, '#f34e56', 2000);
						valid_flag = 0;
					}
				} 
				
				if ($('#soa-property-11').length && !$('#soa-property-11').val()) {
					$('#soa-property-11').addClass('error');
					$('#soa-property-11').parent().find('label.error').detach();
					$('#soa-property-11').after('<label class="error">Поле "Улица" не заполнено!</label>');
					valid_flag = 0;
				}
				
				if ($('#soa-property-12').length && !$('#soa-property-12').val()) {
					$('#soa-property-12').addClass('error');
					$('#soa-property-12').parent().find('label.error').detach();
					$('#soa-property-12').after('<label class="error">Поле "Дом" не заполнено!</label>');
					valid_flag = 0;
				}
				
				
				if ($('#soa-property-1').length && !$('#soa-property-1').val()) {
					$('#soa-property-1').addClass('error');
					$('#soa-property-1').parent().find('label.error').detach();
					$('#soa-property-1').after('<label class="error">Поле "Имя" не заполнено!</label>');
					valid_flag = 0;
				}
				
				if (!$('#soa-property-3').val()) {
					$('#soa-property-3').addClass('error');
					$('#soa-property-3').parent().find('label.error').detach();
					$('#soa-property-3').after('<label class="error">Поле "Телефон" не заполнено!</label>');
					valid_flag = 0;
				} else {
					let selected_country_name = $('#country_select_new option:selected').text();

					if (selected_country_name == "Беларусь") {	
						if (!(/^\+375[0-9]{1,}$/.test($('#soa-property-3').val()))){
							$('#soa-property-3').addClass('error');
							$('#soa-property-3').parent().find('label.error').detach();
							$('#soa-property-3').after('<label class="error">Поле "Телефон" заполнено неверно!</label>');
							valid_flag = 0;
						} 
					} else if (selected_country_name == "Казахстан") {	
						if (!(/^\+7[0-9]{1,}$/.test($('#soa-property-3').val()))){
							$('#soa-property-3').addClass('error');
							$('#soa-property-3').parent().find('label.error').detach();
							$('#soa-property-3').after('<label class="error">Поле "Телефон" заполнено неверно!</label>');
							valid_flag = 0;
						} 
					} else if (selected_country_name == "Россия") {
						//if (!(/^\+7[0-9\s]{10}$/.test($('#soa-property-3').val()))){
						if (!(/^\+7\s\([0-9]{3}\)\s[0-9]{3}\-[0-9]{2}\-[0-9]{2}$/.test($('#soa-property-3').val()))){	
							$('#soa-property-3').addClass('error');
							$('#soa-property-3').parent().find('label.error').detach();
							$('#soa-property-3').after('<label class="error">Поле "Телефон" заполнено неверно!</label>');
							valid_flag = 0;
						...
						if (!(/^[\+]{0,1}[0-9]*$/.test($('#soa-property-3').val()))){
							$('#soa-property-3').addClass('error');
							$('#soa-property-3').parent().find('label.error').detach();
							$('#soa-property-3').after('<label class="error">Поле "Телефон" заполнено неверно!</label>');
							valid_flag = 0;
				...
				if (valid_flag) {
					BX.Sale.OrderAjaxComponentExt.setCart()
				}
