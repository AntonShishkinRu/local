...
if(isset($arResult["PROPERTIES"]["MORE_PHOTO"]["VALUE"]) && is_array($arResult["PROPERTIES"]["MORE_PHOTO"]["VALUE"])) {
	foreach($arResult["PROPERTIES"]["MORE_PHOTO"]["VALUE"] as $arFile) {
...
if (empty($arResult["MORE_PHOTO"]) && !empty($arResult["PREVIEW_PICTURE"]["SRC"]) && $arResult["PREVIEW_PICTURE"]["SRC"]) 
	$arResult["MORE_PHOTO"][] = $arResult["PREVIEW_PICTURE"];

$dataClassCatalog = \Bitrix\Iblock\Iblock::wakeUp($arResult["IBLOCK_ID"])->getEntityDataClass();
?>
<div class="bx-catalog-element product_info block" id="<?=$itemIds['ID']?>" itemscope itemtype="http://schema.org/Product">
	<div class="cont row">
		<? if (!empty($arResult["MORE_PHOTO"])) { ?>
			<? $arVideos = array();
				if (isset($arResult["PROPERTIES"]["VIDEO"]["VALUE"][0])) { 
					foreach($arResult["PROPERTIES"]["VIDEO"]["VALUE"] as $key => $val) {
						if ($tmp_num = intval($arResult["PROPERTIES"]["VIDEO"]["DESCRIPTION"][$key])) {
							$arVideos[$tmp_num - 1] = $val;
						}
					}
				}
				?> 
			<div class="images">
				<div class="thumbs">
					<div class="swiper">
						<div class="swiper-wrapper">
							<? foreach($arResult["MORE_PHOTO"] as $key => $arItem) { ?>
								<div class="swiper-slide">
									<div class="image">
										<?if (isset($arVideos[$key])) { ?>
											<div class="icon">
												<svg><use xlink:href="/local/images/sprite.svg#ic_video_play"></use></svg>
											</div>
										<?}?>

										<img src="<?=$arItem['SRC']?>" alt="<?=$name?>" loading="lazy">
									</div>
								</div>
							<? } ?>
						</div>
				...
				<div class="big">
					<? if (($cnt = count($arVideos)) || $price['DISCOUNT']) { ?>
						<div class="stickers">
							<? if ($price['DISCOUNT']) { ?>
							...
							<? if ($cnt) { ?>
								<div class="sticker">
									<svg class="icon"><use xlink:href="/local/images/sprite.svg#ic_video_play"></use></svg>
									<span>ВИДЕО</span>
								</div>
							<? } ?>
						</div>
								...
								<div class="swiper-slide">
									<?if (isset($arVideos[$key])) { ?>
										<div class="video-container">
											<iframe src="<?=$arVideos[$key];?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
										</div>
									<?} else { ?>
										<div class="image">
											<img src="<?=$arItem['SRC']?>" alt="<?=$name?>" loading="lazy">
		...
		<div class="data">
			...
			<? if (!empty($arResult['DISPLAY_PROPERTIES']['MODEL'])) { ?>
				<? if (!empty($arResult['DISPLAY_PROPERTIES']['MODEL']['~VALUE'])) { ?>
					<div class="desc"><?=$arResult['DISPLAY_PROPERTIES']['MODEL']['~VALUE'];?></div>
				<? } 
				unset($arResult['DISPLAY_PROPERTIES']['MODEL']);
				?>
			<? } ?>
			...
			<div class="price" id="<?=$itemIds['PRICE_ID']?>">
				...
				if ($arParams['SHOW_OLD_PRICE'] === 'Y')
				{
					...
					<span class="old" id="<?=$itemIds['OLD_PRICE_ID']?>"
					...
					<span class="item_economy_price" id="<?=$itemIds['DISCOUNT_PRICE_ID']?>"
						style="display: <?=($showDiscount ? 'none' : 'none')?>;">
						<?php
						if ($showDiscount)
						{
			...
			<div class="credit">
				<span>Или по x xxx руб. в 4 беспроцентных платежа с</span>

				<img src="/local/images/ic_credit.svg" alt="" loading="lazy">
			</div>
			...
			<div class="btns" data-entity="main-button-container">
				<button class="buy_btn buy_btn_el" prodid="<?=$actualItem['ID'];?>" onclick="add2cart(this); return false;"><?=$arParams['MESS_BTN_ADD_TO_BASKET']?></button>
				<?php
				if ($showSubscribe)
				{
					?>
				...
				<button class="favorite_btn favorite_link_prod" prod="<?=$actualItem['ID'];?>" onclick="add2fav(<?=$actualItem['ID'];?>, this); return false;">
			...
			<? if (!empty($arResult['DISPLAY_PROPERTIES']['ANOTHER_COLORS']['LINK_ELEMENT_VALUE'])) { ?>
				<div class="colors">
					<div class="label">Модель в другом цвете</div>

					<div class="swiper">
						<div class="swiper-wrapper">
							<? foreach($arResult['DISPLAY_PROPERTIES']['ANOTHER_COLORS']['LINK_ELEMENT_VALUE'] as $arItem) { ?>
								<div class="swiper-slide">
									<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="color">
										<div class="thumb">
											<? if ($arItem['PREVIEW_PICTURE']) { ?>
												<img src="<?=CFile::GetPath($arItem['PREVIEW_PICTURE']);?>" alt="" loading="lazy">
											<? } ?>
										</div>
										<div>
										<?
											$element = $dataClassCatalog::getByPrimary($arItem['ID'], [
												'select' => ['COLOR_' => 'COLOR.ITEM'],
											])->fetch();
											if (!empty($element['COLOR_VALUE'])) echo $element['COLOR_VALUE'];
			...
			<? 
			unset($arResult['DISPLAY_PROPERTIES']['ANOTHER_COLORS']);
			} ?>
			...
			<div class="info accordion">
							...
							if (!empty($arResult['DISPLAY_PROPERTIES']))
							{
								foreach ($arResult['DISPLAY_PROPERTIES'] as $property) {
									...
									<div>
										...
										<div class="dots"></div>
										<div class="val"><?=(is_array($property['DISPLAY_VALUE'])
				...
				<? if ($showDescription) { ?>
					<div class="accordion_item" data-entity="tab-container" data-value="description"
				...
				$arRes = $dataClassSettings::getList([
					'select' => ['TAB_HOW_TO_BUY_' => 'CARD_HOW_TO_BUY', 'TAB_PAY_' => 'CARD_PAY', 'TAB_DELIVERY_' => 'CARD_DELIVERY'],
					'limit' => 1
				])->fetch(); */
				$arRes = [];
				?>

				<? if ($arRes['TAB_HOW_TO_BUY_VALUE'] && ($text = unserialize($arRes['TAB_HOW_TO_BUY_VALUE'])['TEXT'])) { ?>
					<div class="accordion_item">
						<div class="head">
						...
						<div class="data how_buy">
							<?=$text;?>
						</div>
					</div>
				<? } ?>
				
				<? if ($arRes['TAB_PAY_VALUE'] && ($text = unserialize($arRes['TAB_PAY_VALUE'])['TEXT'])) { ?>
					<div class="accordion_item">
						<div class="head">
				...
				<? if ($arRes['TAB_DELIVERY_VALUE'] && ($text = unserialize($arRes['TAB_DELIVERY_VALUE'])['TEXT'])) { ?>
					<div class="accordion_item">
						<div class="head">
			...
			<div class="as_a_gift">
				<button class="btn modal_btn" data-modal="gift_modal">
					<svg class="icon"><use xlink:href="/local/images/sprite.svg#ic_gift"></use></svg>
...
<?$this->SetViewTarget('gift_product_hidden_inputs');?>
	<?
	$url = ((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
	$url = explode('?', $url);
	$url = $url[0];
	?>
	<input type="hidden" name="prod_url" value="<?=$url?>"/>
	<input type="hidden" name="prod_name" value="<?=$name?>"/>	
<?$this->EndViewTarget();?>
 

<?$this->SetViewTarget('gift_product_info');?>
	<div class="product">
		<? if (isset($arResult["MORE_PHOTO"][0]['SRC'])) { ?>
			<a href="" class="thumb">
				<img src="<?=$arResult["MORE_PHOTO"][0]['SRC'];?>" loading="lazy">
			</a>
		<? } ?>

		<div class="info">
			<? if (!empty($arResult['PROPERTIES']['BRAND']['VALUE'])) { ?>
				<div class="brand"><a href="/catalog/<?=$arResult['PROPERTIES']['BRAND']['VALUE_XML_ID'];?>/"><?=$arResult['PROPERTIES']['BRAND']['VALUE']?></a></div>
			<? } ?>
			...
			<? if (!empty($arResult['PROPERTIES']['ARTICLE']['VALUE'])) { ?>
				<div class="articul">Aртикул: <?=$arResult['PROPERTIES']['ARTICLE']['VALUE'];?></div>
			<? } ?>
			<div class="price">
				<?=$price['PRINT_RATIO_PRICE']?> <?php
				if ($arParams['SHOW_OLD_PRICE'] === 'Y') { ?>
					<span class="old" id="<?=$itemIds['OLD_PRICE_ID']?>"
						style="display: <?=($showDiscount ? '' : 'none')?>;">
						<?=($showDiscount ? $price['PRINT_RATIO_BASE_PRICE'] : '')?>
